...
export function parseCidr(cidr: string): string[] {
  ...
  if (!Number.isFinite(prefixLength) || prefixLength < CIDR_MAX_BITS || prefixLength > CIDR_MIN_BITS) {
...
export function parsePorts(input: string | number[]): number[] {
  ...
  if (normalized.includes('-')) {
    ...
    if (!Number.isFinite(start) || !Number.isFinite(end) || start < PORT_FLOOR || end > PORT_CEIL || start > end) {
      throw new Error(`Port range must be between ${PORT_FLOOR} and ${PORT_CEIL}, and start must be less than or equal to end`)
    }
