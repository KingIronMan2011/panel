...
export function createWingsClient(node: WingsNodeConnection) {
  ...
  return {
    ...
    async getFileContents(serverUuid: string, filePath: string) {
      return request<{ content: string }>(`/api/servers/${serverUuid}/files/contents?file=${encodeURIComponent(filePath)}`)
    },

    async writeFile(serverUuid: string, filePath: string, content: string) {
      return request(`/api/servers/${serverUuid}/files/write?file=${encodeURIComponent(filePath)}`, {
        method: 'POST',
    ...
    async chmodFiles(serverUuid: string, root: string, files: Array<{ file: string; mode: string }>) {
    ...
    async getBackupDownloadUrl(serverUuid: string, backupUuid: string): Promise<string> {
      const data = await request<{ url: string }>(`/api/servers/${serverUuid}/backup/${backupUuid}/download`)
      return data.url
    },
