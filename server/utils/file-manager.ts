...
export class FileManager {
  ...
  async writeFile(serverUuid: string, filePath: string, content: string, options: FileManagerOptions = {}) {
    const sanitizedPath = this.sanitizePath(filePath)
    ...
    try {
      ...
      await client.renameFile(serverUuid, directory, [
  ...
  async deleteFiles(serverUuid: string, directory: string, files: string[], options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
    const { client, server } = await getWingsClientForServer(serverUuid)
  ...
  async renameFiles(serverUuid: string, directory: string, renames: Array<{ from: string; to: string }>, options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
    const { client, server } = await getWingsClientForServer(serverUuid)
  ...
  async createDirectory(serverUuid: string, directory: string, name: string, options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
  ...
  async compressFiles(serverUuid: string, directory: string, files: string[], options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
    const { client, server } = await getWingsClientForServer(serverUuid)
  ...
  async decompressFile(serverUuid: string, directory: string, file: string, options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
    const { client, server } = await getWingsClientForServer(serverUuid)
  ...
  async chmodFiles(serverUuid: string, directory: string, files: Array<{ file: string; mode: string }>, options: FileManagerOptions = {}) {
    const sanitizedDir = this.sanitizePath(directory)
    const { client, server } = await getWingsClientForServer(serverUuid)
