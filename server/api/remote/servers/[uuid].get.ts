...
function safeJsonParse(value: string | null | undefined, defaultValue: unknown = {}): unknown {
  ...
  try {
    ...
    debugWarn(`[Wings Config] safeJsonParse failed:`, error instanceof Error ? error.message : String(error), `Value:`, value.substring(0, 100))
    return defaultValue
  }
}
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const settings = {
    ...
    container: (() => {
      const selectedImage = server.dockerImage || egg?.dockerImage || server.image || 'ghcr.io/pterodactyl/yolks:latest'
    ...
    for (const [file, data] of Object.entries(configFiles)) {
      if (data && typeof data === 'object' && 'parser' in data) {
        ...
        if (fileData.find && typeof fileData.find === 'object') {
          const processedFind = processFindValues(fileData.find, serverStructure) as Record<string, unknown>
